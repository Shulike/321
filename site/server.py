...
class Handler(server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        path = parsed.path
        if path == '/login':
            self.show_login(message=msg)
            return
        if path == '/register':
            self.show_register(message=msg)
            return
        if path == '/logout':
            self.handle_logout()
            return
        if path == '/profile':
            if not self.require_login():
                return
            self.show_profile(message=msg)
            return
        if path == '/' or path.startswith('/dashboard'):
            if not self.require_login():
                return
            term = query.get('q', [''])[0]
            self.show_dashboard(search=term, message=msg)
        elif self.path == '/create':
            if not self.require_login():
                return
            self.show_create(message=msg)
            ...
            if not self.require_login():
                return
            self.show_details(aid, message=msg)
            ...
            if not self.require_login():
                return
            self.show_edit(aid, message=msg)
    ...
    def show_dashboard(self, search='', message=''):
        assistants = load_assistants()
        if search:
            term = search.lower()
            assistants = [a for a in assistants
                          if term in a['name'].lower() or term in a.get('description','').lower()]
        rows = []
        for a in assistants:
            actions = (
    ...
    def show_create(self, message=''):
        msg_html = f"<div class='alert alert-info'>{message}</div>" if message else ''
        html = render_template('create.html', username=self.user, message=msg_html)
        self.send_response(200)
        self.send_header('Content-Type', 'text/html; charset=utf-8')
        self.end_headers()
        self.wfile.write(html.encode('utf-8'))
    ...
    def handle_create(self):
        ...
        self.send_response(303)
        self.send_header('Location', '/?msg=Assistant+created')
        self.end_headers()

    def show_edit(self, aid, message=''):
        assistants = load_assistants()
    ...
    def handle_edit(self, aid):
        ...
        save_assistants(assistants)
        self.send_response(303)
        self.send_header('Location', f'/assistant/{aid}?msg=Updated')
        self.end_headers()
    ...
    def show_login(self, message=''):
        msg_html = f"<div class='alert alert-info'>{message}</div>" if message else ''
        html = render_template('login.html', message=msg_html)
        self.send_response(200)
        self.send_header('Content-Type', 'text/html; charset=utf-8')
        self.end_headers()
        self.wfile.write(html.encode('utf-8'))
    ...
    def handle_login(self):
        ...
        if any(u['username'] == username and u['password'] == hash_password(password) for u in users):
            ...
            self.send_response(303)
            self.send_header('Location', '/?msg=Login+successful')
            self.send_header('Set-Cookie', f'session={sid}; HttpOnly')
            self.end_headers()
        else:
            self.send_response(303)
            self.send_header('Location', '/login?msg=Invalid+credentials')
            self.end_headers()

    def show_register(self, message=''):
        msg_html = f"<div class='alert alert-info'>{message}</div>" if message else ''
        html = render_template('register.html', message=msg_html)
        self.send_response(200)
        self.send_header('Content-Type', 'text/html; charset=utf-8')
        self.end_headers()
        self.wfile.write(html.encode('utf-8'))
    ...
    def handle_register(self):
        ...
        if any(u['username'] == username for u in users) or not username or not password:
            self.send_response(303)
            self.send_header('Location', '/register?msg=Invalid+details')
            self.end_headers()
            return
    ...
    def handle_logout(self):
        ...
        if sid and sid in SESSIONS:
            del SESSIONS[sid]
        self.send_response(303)
        self.send_header('Location', '/login?msg=Logged+out')
        self.send_header('Set-Cookie', 'session=; expires=Thu, 01 Jan 1970 00:00:00 GMT')
        self.end_headers()

    def show_profile(self, message=''):
        msg_html = f"<div class='alert alert-info'>{message}</div>" if message else ''
        html = render_template('profile.html', username=self.user, message=msg_html)
        self.send_response(200)
        self.send_header('Content-Type', 'text/html; charset=utf-8')
        self.end_headers()
        self.wfile.write(html.encode('utf-8'))
    ...
    def handle_change_password(self):
        ...
        if not password:
            self.send_response(303)
            self.send_header('Location', '/profile?msg=Password+required')
            self.end_headers()
            return
        users = load_users()
        for u in users:
        ...
        save_users(users)
        self.send_response(303)
        self.send_header('Location', '/?msg=Password+updated')
        self.end_headers()

    def show_details(self, aid, message=''):
        assistants = load_assistants()
        a = next((x for x in assistants if x['id'] == aid), None)
        if not a:
            self.send_error(404)
